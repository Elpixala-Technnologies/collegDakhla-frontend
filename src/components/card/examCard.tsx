...
export default function ExamCard({ featuredExams }: any) {
  ...
  const bannerUrl = featuredExams?.attributes?.banner?.data[0]
    ? getStrapiMedia(
        featuredExams?.attributes?.banner?.data[0]?.attributes?.url
      )
