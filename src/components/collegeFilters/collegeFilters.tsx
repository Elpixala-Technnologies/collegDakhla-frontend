	...
	return (
		<>
			...
			</div >
			{
				...
				params.isMobile ? (<>
					<div className="fixed top-0 h-2/6 w-full bg-black opacity-2" onClick={params.handleMobileFilter}></div>
					<div className="opacity-100 z-50 block fixed right-0 bottom-0 left-0 w-screen h-4/6 bg-white text-black rounded-xl">
						...
						<div className="filter-body">
									...
									>
										...
										<TabPanel value={value} index={0}>
											{streamsData?.streams?.data?.map((stream: any) => {
												return (
													<div key={stream.id} className="flex gap-1 items-center my-2 cursor-pointer">
														<input
															type="radio"
															id={stream.id}
														...
														/>
														<span className="text-xxs font-semibold text-secondary-text hover:text-primary">{stream.attributes.streamName}</span>
													</div>
												)
											})
											}
							...
							<div className="filter-buttons flex justify-center w-full gap-14 px-5 bg-white">
								<div className="w-1/2 border border-gray-300" onClick={resetFilters}>
									<Button href={""} text="Cancel" filled fontSize="text-sm" fontWeight="font-bold" width="w-full" align="text-center" bgColor="bg-white" fontColor="text-primary" />
								</div>
								<div className="w-1/2" onClick={params.handleMobileFilter}>
									<Button href={""} text="Apply Filters" filled fontSize="text-sm" fontWeight="font-bold" width="w-full" align="text-center" bgColor="bg-primary" fontColor="text-white" />
								</div>
							</div>
						</div>
					</div>
